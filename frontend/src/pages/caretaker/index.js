  ...
  return (
    <div className="h-screen font-mon flex flex-col items-center  ">
      ...
      {list.length > 0 && <ul>
        {list.map((item) => (
          <li key={item.id}>
            <ListItem relation={item.relation} name={item.name} phone={item.phone} buttonName="PING" handles={() => ping(item)} />
          </li>
        ))}
      ...
      {(list.length == 0) && <div>
        ...
        <h1>You're not connected to any patients, yet.<br />
          Get started by connecting to your patient.</h1>
      </div>}
